...
def write_authoring_artifacts(artifact_root: str | Path, state: dict[str, Any]) -> list[str]:
    ...
    # ── P2: Consolidated Excel Workbook ──
    wb_path = root / "CER_Workbook.xlsx"
    try:
        ...
        for fname in OUTPUT_FILES:
            if not fname.endswith(".xlsx"):
                continue
            data = payloads.get(fname.replace(".xlsx", ".json"), state.get(fname.replace(".xlsx", "")))
            if not isinstance(data, list) or not data:
                continue
            sheet_name = fname.replace(".xlsx", "")[:31]  # Excel 31-char limit
        ...
        wb.save(str(wb_path))
        written.append(f"CONSOLIDATED_WORKBOOK: {wb_path} ({len(sheet_names)} sheets)")
...
def _add_markdown_to_doc(doc: Any, markdown: str) -> None:
    """Convert markdown to DOCX with TOC, page breaks, and professional formatting."""
    from docx.enum.text import WD_ALIGN_PARAGRAPH
    from docx.oxml.ns import qn

    # Add title page
    title = doc.add_heading("Clinical Evaluation Report", level=0)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph("")
    doc.add_paragraph("Document Control: This is an auto-generated CER draft for review.")
    doc.add_page_break()

    # Add Table of Contents placeholder
    doc.add_heading("Table of Contents", level=1)
    toc_para = doc.add_paragraph("(Right-click and select 'Update Field' to generate TOC in Word)")
    doc.add_page_break()
    ...
    while idx < len(lines):
        ...
        if stripped.startswith("# ") and not stripped.startswith("## "):
            if idx > 5:  # not the very first heading
                doc.add_page_break()
            if "Annex" in stripped:
                in_annex = True
        ...
        if stripped.startswith("#"):
            ...
            heading = doc.add_heading(stripped.lstrip("#").strip(), level=level)
            # Set TOC outline level for Word TOC generation
    ...
    # Add document footer
    doc.add_page_break()
    doc.add_paragraph("— End of Clinical Evaluation Report —")
    doc.add_paragraph("This document was generated by the DeerFlow CER Authoring System and requires human review before regulatory submission.")
